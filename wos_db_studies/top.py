...
import logging
from wos_db_studies.util.db import get_arangodb_client, delete_collections, define_collections, upsert_docs_batch, \
    insert_edges_batch
from wos_db_studies.util.pjson import parse_config
from wos_db_studies.utils import merge_doc_basis
from wos_db_studies.utils_json import parse_edges, process_document_top
...
def ingest_json_files(fpath, config, protocol="http", ip_addr="127.0.0.1", port=8529, database="_system",
    ...
    vcollections, vmap, graphs, index_fields_dict, eindex = parse_config(config=config,
                                                                         prefix=prefix)
    ...
    files = sorted([f for f in listdir(fpath) if isfile(join(fpath, f)) and keyword in f])
...
def ingest_json(json_data, config, prefix, sys_db=None, dry=False):
    ...
    logger.info(f" converting json to vertices and edges took {t_parse.elapsed:.2f} sec")
    ...
    with timer.Timer() as t_ingest:
        ...
        for k in kkey_vertex:
            ...
            query0 = upsert_docs_batch(
                v, vmap[k], index_fields_dict[k], "doc", True
            )
    ...
    with timer.Timer() as t_ingest_edges:
        ...
        for uv in kkey_edge:
            ...
            if len(super_dict[uv]) == 0:
                ...
                logger.error(f" for unknown reason edge batch "
